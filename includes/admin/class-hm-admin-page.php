...
final class HM_MC_Admin_Page {
	...
	public static function render_page() : void {
		...
		<div class="wrap">
				...
				<table class="widefat striped">
					<thead>
						<tr>
							...
							<th style="width: 140px;"><?php echo esc_html__( 'Actions', 'hm-menu-controller' ); ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<h2><?php echo esc_html__( 'Menu Tabs (Preview)', 'hm-menu-controller' ); ?></h2>
			<p><?php echo esc_html__( 'This is a read-only preview of the current admin sidebar menu structure. Next step will add per-user save + hide behavior.', 'hm-menu-controller' ); ?></p>

			<?php self::render_menu_tabs_preview( $menu_tree ); ?>

		</div>
		<?php
	}

	private static function render_menu_tabs_preview( array $menu_tree ) : void {
		if ( empty( $menu_tree ) ) {
		...
		$active = '';
		if ( isset( $_GET['hm_mc_tab'] ) ) {
			$active = sanitize_text_field( wp_unslash( $_GET['hm_mc_tab'] ) );
		}
		...
		foreach ( $menu_tree as $node ) {
			...
			echo '<thead><tr>';
			echo '<th style="width:60px;">' . esc_html__( 'Show', 'hm-menu-controller' ) . '</th>';
